...
//
// Multi-process solution written by Furkan Alaca: 
// closest_parallel(), _closest_parallel(), generate_points(), print_usage(), and main()
...
int main(int argc, char **argv) 
{
    ...
    qsort(P1, n1, sizeof(struct Point), compare_y);
    ...
    //double result_s = brute_force(P1, n1);
    if (clock_gettime(CLOCK_MONOTONIC, &end)) {
        perror("clock_gettime");
        exit(1);
    }
